...
tfs<-read.table(file.path(di, "data/mat_rank_gs_split_sig.txt"),header = T,sep = "\t") %>% as.data.frame()
g<-tfs$gene %>% unique()
...
df.00 <- queryMany(df.tmp$query, scopes="alias", fields=c("symbol"), species="human") %>% as.data.frame() #查找不成功的用别名重新查找。
...
id.2 <- df.00 %>% dplyr::select(query,symbol) 
# id.e <- df.err %>% dplyr::select(query,entrezgene)
id <- bind_rows(id.1,id.2) 

write.table(id,file.path(di,"output/06_all_gene_from_jintao_symbol.txt"),quote = F,sep = "\t",row.names = F)
