            ...
            unless(/^#/){
                ...
                if ($number >1){
                    if (/^miRNAid/){
                        ...
                        my $tissue = "NA";
                        if ($geneType =~/mRNA/){ #$geneType 为mRNA和其他$geneType的表格有些不同。
                            my $narrowStart =$f[6];
                            my $narrowEnd =$f[7];
                            my $broadStart =$f[8];
                            my $broadEnd = $f[9];
                        ...
                        else{ #除$geneType 为mRNA的其他$geneType
                            my $Start2 = $f[6];
                            my $End2 =$f[7];
                            my $Strand2 = $f[8];
                            if (exists $hash1{$k}){ #
                                my @vs =@{$hash1{$k}};
                                foreach my $v(@vs){
                                    my $output = "$v\t$chr\t$Start2\t$End2\t$Strand2\t$geneType\t$interaction_name2\t$ENSG_ID\t$gene_name\tNA\t$Sub_interaction_type\tRNA-RNA\tHg19\t$tissue\t$pancancerNum\tstarBase";
                                    unless(exists $hash2{$output}){
                                        $hash2{$output} =1;
                                        print $O1 "$output\n";
                                    }
                                ...
                                my $v = "NA\tNA\tNA\tNA\tmiRNA\t$miRNAname\tNA\tNA\tNA";
                                my $output = "$v\t$chr\t$Start2\t$End2\t$Strand2\t$geneType\tinteraction_name2\t$ENSG_ID\t$gene_name\tNA\t$Sub_interaction_type\tRNA_RNA\tHg19\t$tissue\t$pancancerNum\tstarBase";
                                unless(exists $hash2{$output}){
                                    $hash2{$output} =1;
                                    print $O1 "$output\n";
                                }
