    ...
    foreach my $QTL2(@QTL2s){
        unless ($QTL1 =~/$QTL2/){#防止自己与自己取交集
            ...
            unless(exists $hash1{$cp_format1} || exists $hash1{$cp_format2} ){
                ...
                my $fo1 = "../output/xQTL_merge/Cis_Trans_1MB/NHPoisson_emplambda_interval_1000_cutoff_7.3_all_merge_1MB_cis_${QTL1}_trans_${QTL2}.txt.gz";
                open my $O1, "| gzip >$fo1" or die $!;
                print $O1 "cis_${QTL1}_emplambda\ttrans_${QTL2}_emplambda\tpos\tchr\n";
                my $fo2 = "../output/xQTL_merge/Cis_Trans_10MB/NHPoisson_emplambda_interval_1000_cutoff_7.3_all_merge_10MB_cis_${QTL1}_trans_${QTL2}.txt.gz";
                open my $O2, "| gzip >$fo2" or die $!;
                print $O2 "cis_${QTL1}_emplambda\ttrans_${QTL2}_emplambda\tpos\tchr\n";
                print "$cp_format1\n";
                my %hash2;
                my %hash3;
                while(<$I1>)
