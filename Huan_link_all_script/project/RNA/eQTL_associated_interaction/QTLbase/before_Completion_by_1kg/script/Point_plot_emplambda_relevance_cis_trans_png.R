...
for (file in files){
    ...
    if (n_r >1){
        ...
        # round(org$x_QTL,4)
        setwd("/home/huanhuan/project/RNA/eQTL_associated_interaction/QTLbase/figure/xQTL_merge/Cis_Trans_1MB/")
...
for (file in files){
    ...
    if (n_r >1){
        ...
        colnames(org)[2] <- "y_QTL" #change colname name 
        # round(org$x_QTL,4)
        setwd("/home/huanhuan/project/RNA/eQTL_associated_interaction/QTLbase/figure/xQTL_merge/Cis_Trans_10MB/")
        figure_name <-name <- str_replace(file,".txt.gz",".png")
        png(figure_name,height = 1000,width = 1000)
