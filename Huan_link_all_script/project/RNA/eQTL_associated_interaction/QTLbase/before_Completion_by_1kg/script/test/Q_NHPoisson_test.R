...
org2 <- filter(org,QTL_type == "eQTL" & Population == "EUR")
...
org1<-org1[order(org1$SNP_pos), ]
BarEv <- POTevents.fun(T = -log10(org1$min_p), thres = 7, date = org1$SNP_pos)
...
write.table(rs1,"../output/NHPoisson_POTevents_Q_snp_unique_all_22.txt",row.names = F, col.names = T,quote =F,sep="\t")
write.table(rs2,"../output/NHPoisson_emplambda_Q_snp_unique_all_22.txt",row.names = F, col.names = T,quote =F,sep="\t")

#------------------------------------the same snp occur 1 time 
