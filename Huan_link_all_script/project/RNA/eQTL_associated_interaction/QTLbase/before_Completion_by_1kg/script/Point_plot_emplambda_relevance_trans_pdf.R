...
for (file in files){
    ...
    if (n_r >1){
        ...
        # round(org$x_QTL,4)
        setwd("/home/huanhuan/project/RNA/eQTL_associated_interaction/QTLbase/figure/xQTL_merge/Trans_1MB/")
...
for (file in files){
    ...
    if (n_r >1){
        ...
        colnames(org)[2] <- "y_QTL" #change colname name 
        # round(org$x_QTL,4)
        setwd("/home/huanhuan/project/RNA/eQTL_associated_interaction/QTLbase/figure/xQTL_merge/Trans_10MB/")
        figure_name <-name <- str_replace(file,".txt.gz",".pdf")
        pdf(figure_name,height = 7,width = 7)
        p1<-ggplot(org, aes(x =x_QTL, y=y_QTL)) +geom_point(size=0.01, color ="black" ) 
