...
LD<-read.table("03_ALL_annotation_ld_band.txt.gz",header = T,sep = "\t") %>% as.data.frame()
...
starts<-seq(11,length(unique_bind),6)
for (start in starts){
    end = start+5
    unique_bind_head <-unique_bind[start:end]
    print(unique_bind_head)
    rm(re_mc)
    ...
    file <-paste("MCL_split_result/MCL_result_top_",start,"_",end,".txt.gz",sep="")
    gz1 <- gzfile(file, "w")
