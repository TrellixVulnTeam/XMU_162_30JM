##统计../output/NHPoisson_emplambda_interval_${i}_all_eQTLbase.txt中 中每个阶段eqtl的数目，
...
my $fo1 = "../../../output/ALL_trans_eQTL/10MB/count_number_by_emplambda_in_different_interval_cutoff_7_all_trans_eQTL.txt";
open my $O1, '>', $fo1 or die "$0 : failed to open output file '$fo1' : $!\n";
my $header = "emplambda\tnumber\tinterval";
print $O1 "$header\n";

my @intervals;
for (my $i=2000;$i<8001;$i=$i+2000){
    push @intervals, $i;
}
...
foreach my $i(@intervals) { #interval
    my $f1 = "../../../output/ALL_trans_eQTL/10MB/NHPoisson_emplambda_interval_${i}_all_trans_eQTL.txt";
