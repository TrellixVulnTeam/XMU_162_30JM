...
interval<-seq(from=40000, to=100000, by=10000)
cutoff = 7
for (j in interval){
    dir_name = paste("all_eQTLbase_cutoff_",cutoff,"_int",j, sep = "")
    #----------------------------------mkdir_dir
    ...
    output_file1<-paste("../output/NHPoisson_POTevents_Px_cutoff_",cutoff,"_all_eQTLbase.txt", sep = "")
    output_file2<-paste("../output/NHPoisson_emplambda_interval_",j,"_all_eQTLbase.txt", sep = "")
    output_file3<-paste("../output/NHPoisson_POTevents_inddat_cutoff_",cutoff,"_all_eQTLbase.txt", sep = "")
