...
org_pop <- filter(org,QTL_type == "eQTL" & Population == "EUR")
#--------------------500k

interval<-seq(from=10000, to=100000, by=10000)
# interval<-seq(from=20000, to=100000, by=10000)
# interval<-seq(from=10000, to=10000, by=1)
for (j in interval){
    dir_name = paste("cut0ff_9_int",j, sep = "")
    #----------------------------------mkdir_dir
    ...
    # for(i in c(1:2)){
        ...
        BarEv <- POTevents.fun(T = -log10(org1$min_p), thres = 9, date = org1$SNP_pos) #POTevents.fun
        tB <-org1$SNP_pos
        title <-paste ("EUR_eqtl_int",j,"bp_chr",i ,sep = "")
        figure_name <-paste("./cut0ff_9_int",j,"/emplambda_eqtl_EUR_int_",j,"bp_chr",i,".png", sep = "")
        ...
        dev.off()
        # tmp1<-cbind(Pi=BarEv$Pi,Px=BarEv$Px,datePx=BarEv$datePx,Im=BarEv$Im,Ix=BarEv$Ix,L=BarEv$L,inddat=BarEv$inddat,T=BarEv$T,chr=i) %>% as.data.frame()
        # tmp1<-cbind(Pi=BarEv$Pi,Px=BarEv$Px,datePx=BarEv$datePx,Im=BarEv$Im,Ix=BarEv$Ix,L=BarEv$L,inddat=BarEv$inddat,T=BarEv$T,chr=i) %>% as.data.frame()
        ...
        # rs1_final<-unique(rs1)
        tmp2<-cbind(emplambda=emplambdaB$emplambda, t=emplambdaB$t,chr=i) %>% as.data.frame()
        rs2 <- bind_rows(rs2,tmp2)
        # rs2_final<-unique(rs2)
        print(i)
    }
    # output_file1<-paste("../output/NHPoisson_POTevents_interval",j,".txt", sep = "")
    output_file2<-paste("../output/NHPoisson_emplambda_interval_",j,"_cutoff_9",".txt", sep = "")
    write.table(rs1,"../output/NHPoisson_POTevents_Px_cut0ff_9.txt",row.names = F, col.names = T,quote =F,sep="\t")
    write.table(rs3,"../output/NHPoisson_POTevents_inddat_cutoff_9.txt",row.names = F, col.names = T,quote =F,sep="\t")
