...
#----------------------------------
org_pop<-read.table("../../output/merge_QTL_all_QTLtype_pop_1kg_Completion.txt.gz",header = T,sep = "\t") %>% as.data.frame()
# org_pop <- filter(org,QTL_type == "QTL")
# org_pop <- org
#--------------------500k
...
for (cutoff in cutoffs){
    for (j in interval){
        dir_name = paste("all_QTL_cutoff_",cutoff,"_int",j, sep = "")
        #----------------------------------mkdir_dir
        ...
        for(i in c(1:22)){
            ...
            figure_name <-paste(dir_name,"/emplambda_all_QTL_int_",j,"bp_chr",i,".png", sep = "")
        ...
        output_file1<-paste("../../output/ALL_QTL/NHPoisson_POTevents_Px_cutoff_",cutoff,"_all_QTL.txt", sep = "")
        output_file2<-paste("../../output/ALL_QTL/NHPoisson_emplambda_interval_",j,"cutoff_",cutoff,"_all_QTL.txt", sep = "")
        output_file3<-paste("../../output/ALL_QTL/NHPoisson_POTevents_inddat_cutoff_",cutoff,"_all_QTL.txt", sep = "")
