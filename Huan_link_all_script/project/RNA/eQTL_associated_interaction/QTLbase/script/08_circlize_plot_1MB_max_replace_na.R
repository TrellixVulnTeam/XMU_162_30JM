...
data<- spread(org_a,xQTL,max_emplambda)
nrow(data)
data1<-data
data1[is.na(data1)]=0
data2<-gather(data1,xQTL,max_emplambda,-chr,-region) #the fuction reverse with spread
#-----------------------
...
for(QTL in QTLs){
  ...
  if(QTL=="QTL"|| QTL=="eQTL"||QTL=="mQTL"||QTL=="sQTL"||QTL=="caQTL"||QTL=="hQTL"||QTL=="pQTL"||QTL=="edQTL"){
    ...
    #-----------------------------------------
    pdf("histogram_max_Emplambda_less_na_replace.pdf")
...
#---------
setwd("/home/huanhuan/project/RNA/eQTL_associated_interaction/QTLbase/figure/")
pdf(file = "emplambda_circos_heatmap_mean_interval_1MB_max_min_sacle_outlier_10_na_replace.pdf", height = 8, width = 8, compress = TRUE)
par(mar= c(1,1,1,1))
