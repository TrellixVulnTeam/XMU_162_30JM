...
pwd_dir = "/home/huanhuan/project/RNA/eQTL_associated_interaction/QTLbase/figure/ALL_eQTL"
...
org<-read.table("../../output/1234.txt",header = T,sep = "\t") %>% as.data.frame()
...
for (cutoff in cutoffs){
    for (j in interval){
        ...
        for(i in c(1:22)){
            ...
            figure_name <-paste(dir_name,"/emplambda_all_eQTL_int_",j,"bp_chr",i,".png", sep = "")
        ...
        output_file2<-paste("../../output/ALL_eQTL/NHPoisson_emplambda_interval_",j,"cutoff_",cutoff,"_all_eQTL.txt", sep = "")
        output_file3<-paste("../../output/ALL_eQTL/NHPoisson_POTevents_inddat_cutoff_",cutoff,"_all_eQTL.txt", sep = "")
