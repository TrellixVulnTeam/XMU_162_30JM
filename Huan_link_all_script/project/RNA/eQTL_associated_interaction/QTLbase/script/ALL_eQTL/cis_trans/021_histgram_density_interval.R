...
for(j in intervals){
    ...
    setwd("/home/huanhuan/project/RNA/eQTL_associated_interaction/QTLbase/output/ALL_eQTL/cis_trans/NHP/")
    for (type in types){
    ...
    pdf(paste0("histogram_plot_cutoff_7.3_interval_",j,"_cis_trans.pdf"),height = 9,width = 8) 
    p1<-marrangeGrob(figure_list,nrow=2,ncol=2)  
    print(p1)
    dev.off()

    pdf(paste0("density_plot_cutoff_7.3_interval_",j,"_cis_trans.pdf"),height = 9,width = 8) 
    p1<-marrangeGrob(Density_figure_list,nrow=2,ncol=2)  
    print(p1)
    dev.off()
    # print(j)
}
