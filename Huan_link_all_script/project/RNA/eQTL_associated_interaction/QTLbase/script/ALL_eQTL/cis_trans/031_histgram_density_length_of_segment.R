...
ProcessBedGz <- function(j = NULL){
    for(group in groups){
        ...
        i=1
        setwd("/home/huanhuan/project/RNA/eQTL_associated_interaction/QTLbase/output/ALL_eQTL/cis_trans/")
        for (type in types){ 
            ...
            title_name<- new_type                
                # print(p1)
            Density_figure_list[[i]] <- ggplot(org, aes(x =count)) + geom_density()+
                xlab("Length of segment") +ylab("Density")+p_theme+
                ggtitle(title_name)+theme(plot.title = element_text(hjust = 0.5))
                
            figure_list[[i]] <-ggplot(org, aes(x =count)) +geom_histogram(position="identity")+
        ...
        pdf(paste0(group,"/Density_plot_length_of_segment_",group,"_cutoff_7.3_interval_",j,".pdf"),height = 7.5,width = 8) 
        p1<-marrangeGrob(Density_figure_list,nrow=2,ncol=2,top = top_title)  
        print(p1)
        dev.off()
        #------------

        pdf(paste0(group,"/Histogram_plot_length_of_segment_",group,"_cutoff_7.3_interval_",j,".pdf"),height = 7.5,width = 8) 
