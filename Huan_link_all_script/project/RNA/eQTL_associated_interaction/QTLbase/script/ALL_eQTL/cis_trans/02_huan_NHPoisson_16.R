...
for(file in files){
    ...
    print(file)
    output_file1<-paste("./NHP_16/",name,"/NHPoisson_POTevents_Px_cutoff_",cutoff,"_",name,"_eQTL.txt.gz", sep = "")
    output_file2<-paste("./NHP_16/",name,"/NHPoisson_emplambda_interval_",j,"_cutoff_",cutoff,"_",name,"_eQTL.txt.gz", sep = "")
    output_file3<-paste("./NHP_16/",name,"/NHPoisson_POTevents_inddat_cutoff_",cutoff,"_",name,"_eQTL.txt.gz", sep = "")
...
#     org <- read.table(file,header = T,sep = "\t") %>% as.data.frame()
#     number_list <- c(10:20)
#     aaaaa <- lapply(number_list, function(num){
