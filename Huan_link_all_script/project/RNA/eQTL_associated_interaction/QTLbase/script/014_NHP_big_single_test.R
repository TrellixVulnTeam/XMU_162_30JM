...
ProcessBedGz <- function(j = NULL, org = NULL, cutoff = NULL, name = NULL){
    ...
    for(i in c(1:22)){
        ...
        org1<-dplyr::summarise(org_pg,min_p = min(Pvalue))%>%as.data.frame()
        rm(org_pg)
        #-----------------------------------
    ...
    print(file)
    output_file1<-paste("./ALL_",name,"/NHPoisson_POTevents_Px_cutoff_",cutoff,"_",name,".txt", sep = "")
    output_file2<-paste("./ALL_",name,"/NHPoisson_emplambda_interval_",j,"_cutoff_",cutoff,"_",name,".txt", sep = "")
    output_file3<-paste("./ALL_",name,"/NHPoisson_POTevents_inddat_cutoff_",cutoff,"_",name,".txt", sep = "")
...
setwd("~/project/RNA/eQTL_associated_interaction/QTLbase/output/")
cutoff =7.3
...
#         ProcessBedGz(j = num, org = org, cutoff = cutoff, name = QTL)
#     }, mc.cores = 4)
#     print("aaa")
# }
