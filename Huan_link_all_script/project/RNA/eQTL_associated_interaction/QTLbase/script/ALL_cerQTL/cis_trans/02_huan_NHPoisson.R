...
files<- list.files("~/project/RNA/eQTL_associated_interaction/QTLbase/output/ALL_cerQTL/cis_trans/","txt.gz")
# n<-length(files)

cutoff =7.3
j=1000
for (file in files){
    name <- str_replace(file,"_cerQTL_pop_1kg_Completion.txt.gz","")  #replace
    setwd("~/project/RNA/eQTL_associated_interaction/QTLbase/output/ALL_cerQTL/cis_trans/")
    ...
    print(file)
    output_file1<-paste("NHPoisson_POTevents_Px_cutoff",cutoff,name,"cerQTL.txt", sep = "_")
    output_file2<-paste("NHPoisson_emplambda_interval",j,"cutoff",cutoff,name,"cerQTL.txt", sep = "_")
    output_file3<-paste("NHPoisson_POTevents_inddat_cutoff",cutoff,name,"cerQTL.txt", sep = "_")
