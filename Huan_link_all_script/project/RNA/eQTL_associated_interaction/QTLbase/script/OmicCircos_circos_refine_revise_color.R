...
## main ##
circos_refine <- function (mapping=mapping, xc=400, yc=400, R=400, W=W,  
                           ...
                           lwd=1, col=rainbow(10, alpha=0.8)[7], side="",huan_col=""
){
  ...
  if (type == "heatmap"){
    ...
    rbPal <- colorRampPalette(huan_col)
    ...
    if (col.bar){
      if (col.bar.po == "topleft"){
        color.bar(40,700,50,780, v.min=dat.min, v.max=dat.max,huan_col=huan_col);
      }
      if (col.bar.po == "topright"){
        color.bar(770,700,780,780, v.min=dat.min, v.max=dat.max,huan_col=huan_col);
      }
      if (col.bar.po == "bottomleft"){
        color.bar(40, 10, 50, 90, v.min=dat.min, v.max=dat.max,huan_col=huan_col);
      }
      if (col.bar.po == "bottomright"){
        color.bar(770, 10, 780, 90, v.min=dat.min, v.max=dat.max,huan_col=huan_col);
      }
    }
    if (cluster){
...
color.bar <- function(xl, yb, xr, yt, v.min, v.max,huan_col) {
  
  nticks <- 11; 
  ticks  <- seq(v.min, v.max, len=nticks);
  
  # lut   <- colorRampPalette(c("white","grey50","grey20"))(100);
  lut   <- colorRampPalette(huan_col)(100);
...
save(circos_refine,file = "OmicCircos_circos_refine.R")