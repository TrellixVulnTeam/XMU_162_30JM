...
setwd("/home/huanhuan/project/RNA/eQTL_associated_interaction/QTLbase/figure/ALL_trans_eQTL/1MB")
...
org_pop <- filter(org,QTL_type == "eQTL" && cis_trans_1MB == "trans") #1MB
# org_pop <- org
#--------------------500k
...
cutoff = 7
for (j in interval){
    dir_name = paste("all_1MB_trans_eQTL_cutoff_",cutoff,"_int",j, sep = "")
    #----------------------------------mkdir_dir
    ...
    for(i in c(1:22)){
        ...
        figure_name <-paste(dir_name,"/emplambda_all_trans_1MB_eQTL_int_",j,"bp_chr",i,".png", sep = "")
    ...
    output_file1<-paste("../../../output/ALL_trans_eQTL/1MB/NHPoisson_POTevents_Px_cutoff_",cutoff,"_all_trans_eQTL.txt", sep = "")
    output_file2<-paste("../../../output/ALL_trans_eQTL/1MB/NHPoisson_emplambda_interval_",j,"_all_trans_eQTL.txt", sep = "")
    output_file3<-paste("../../../output/ALL_trans_eQTL/1MB/NHPoisson_POTevents_inddat_cutoff_",cutoff,"_all_trans_eQTL.txt", sep = "")
