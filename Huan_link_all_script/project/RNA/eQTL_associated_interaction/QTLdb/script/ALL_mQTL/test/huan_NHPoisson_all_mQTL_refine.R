...
pwd_dir = "/home/huanhuan/project/RNA/eQTL_associated_interaction/QTLbase/figure/ALL_mQTL"
...
org_pop <- filter(org,QTL_type == "mQTL")
# org_pop <- org
# rm(org)
#--------------------500k
...
for (cutoff in cutoffs){
    for (j in interval){
        dir_name = paste("all_mQTL_cutoff_",cutoff,"_int",j, sep = "")
        #----------------------------------mkdir_dir
        ...
        for(i in c(1:22)){
            ...
            #-------------------------------------- snp has unique p
            org2$SNP_chr<-as.numeric(org2$SNP_chr)
            org2$SNP_pos<-as.numeric(org2$SNP_pos)
            ...
            figure_name <-paste(dir_name,"/emplambda_all_mQTL_int_",j,"bp_chr",i,".png", sep = "")
        ...
        output_file1<-paste("../../output/ALL_mQTL/NHPoisson_POTevents_Px_cutoff_",cutoff,"_all_mQTL.txt", sep = "")
        output_file2<-paste("../../output/ALL_mQTL/NHPoisson_emplambda_interval_",j,"cutoff_",cutoff,"_all_mQTL.txt", sep = "")
        output_file3<-paste("../../output/ALL_mQTL/NHPoisson_POTevents_inddat_cutoff_",cutoff,"_all_mQTL.txt", sep = "")
