...
foreach my $interval(@intervals){
    ...
    my $fo1 = "/state/partition1/huan/QTLdb/output/ALL_eQTL/cis_trans/fisher_exact_test_extend_100bp/fisher_result/interval_${interval}/07_prepare_fisher_test.txt";
    open my $O1, '>', $fo1 or die "$0 : failed to open output file '$fo1' : $!\n";
    # open my $O1, "| gzip >$fo1" or die $!;
    ...
    foreach my $fraction(@fractions){
        foreach my $type(@types){
            my $f1 = "/state/partition1/huan/QTLdb/output/ALL_eQTL/cis_trans/fisher_exact_test_extend_100bp/annotation_out/hotspot/05_annotation_merge_${type}_interval_${interval}_overlap_fraction_${fraction}.txt.gz";
            # open my $I1, '<', $f1 or die "$0 : failed to open input file '$f1' : $!\n"; 
            open( my $I1 ,"gzip -dc $f1|") or die ("can not open input file '$f1' \n"); #读压缩文件
            #--------------------
            my $f2 = "/state/partition1/huan/QTLdb/output/ALL_eQTL/cis_trans/fisher_exact_test_extend_100bp/annotation_out/non_hotspot/05_annotation_merge_${type}_interval_${interval}_overlap_fraction_${fraction}.txt.gz";
            # open my $I1, '<', $f1 or die "$0 : failed to open input file '$f1' : $!\n"; 
