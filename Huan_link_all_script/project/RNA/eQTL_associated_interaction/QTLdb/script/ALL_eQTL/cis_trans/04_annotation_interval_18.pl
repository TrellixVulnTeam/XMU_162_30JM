...
my @intervals = (18);
my @types=("cis_1MB","trans_1MB","cis_10MB","trans_10MB");
my @groups = ("hotspot","non_hotspot");
my @fractions = (0.1,0.9,1);
# my @fractions = (0.1);
    ...
    foreach my $type(@types){
        foreach my $group(@groups){
            foreach my $fraction(@fractions){
                ...
                my $output_dir = "../../../output/ALL_eQTL/cis_trans/fisher_exact_test/annotation/interval_${interval}/${group}/${type}/${fraction}";
                # mkdir $PMID;
                # #------------
                if(-e $output_dir){
