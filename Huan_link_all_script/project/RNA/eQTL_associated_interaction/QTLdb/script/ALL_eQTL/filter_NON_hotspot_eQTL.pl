###QTLs =eQTL在@interval = (6,7,8,9,12,15)时的NON hotspot(segment),得"../output/ALL_${QTL}/un_hotspot/interval_${j}_segment_UNhotspot.txt.gz"; 得point 热点../../output/ALL_${QTL}/un_hotspot/interval_${j}_point_UNhotspot.txt.gz
use warnings;
use strict; 
use utf8;
...
foreach my $QTL(@QTLs){
    ...
    foreach my $j(@interval){
        ...
        my $fo1 = "../../output/ALL_${QTL}/un_hotspot/interval_${j}_segment_UNhotspot.txt.gz";
        open my $O1, "| gzip >$fo1" or die $!;
        # print $O1 "chr\tstart\tend\n";
        my $fo2 = "../../output/ALL_${QTL}/un_hotspot/interval_${j}_point_UNhotspot.txt.gz";
        open my $O2, "| gzip >$fo2" or die $!;
        my $fo3 = "../../output/ALL_${QTL}/un_hotspot/interval_${j}_segment_UNhotspot.bed.gz"; #bed: end=org_end +1;
