 #统计../output/population_specific/NHPoisson_POTevents_inddat_cutoff_${cutoff}_all_QTLbase_${pop}_specific.txt中每个阶段qtl的数目，
...
my @population=("AFR","AMR","EAS","EUR","SAS","MIX");


my $fo1 = "../output/population_specific/count_number_by_emplambda_in_different_interval_cutoff_7_all_QTL_${pop}_specific.txt";
open my $O1, '>', $fo1 or die "$0 : failed to open output file '$fo1' : $!\n";
my $header = "emplambda\tnumber\tinterval";
print $O1 "$header\n";
foreach my $pop (@population){
    for (my $i=10000;$i<70000;$i=$i+10000){ #interval
        my $f1 = "../output/population_specific/NHPoisson_POTevents_inddat_cutoff_${cutoff}_all_QTLbase_${pop}_specific.txt";
