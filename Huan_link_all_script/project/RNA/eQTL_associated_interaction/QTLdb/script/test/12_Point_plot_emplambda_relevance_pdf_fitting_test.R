...
for (file in files){
    ...
    if (n_r >1){
        ...
        # fittings= c("lm","glm","glm")
        # fittings= c("lm","glm")
        # for(fitting in fittings){
            fitting = "glm"
            dir = paste("/home/huanhuan/project/RNA/eQTL_associated_interaction/QTLbase/figure/xQTL_merge/ALL_fitting/ALL_",fitting,sep="")
            # title_name = paste("Cis 1MB", fitting,sep=" ")
            title_name =fitting
            setwd(dir)
            pdf(figure_name,height = 7,width = 7,compress = TRUE)
