    ...
    if ($file =~/$suffix/){ 
        ...
        foreach my $k2(sort keys %hash2){
            ...
            if ($pop =~/mix|eur/){ #混合人群用eur做
                $Population= "EUR";
                $pop = "eur";
                my $command= "plink --bfile /share/data0/1kg_phase3_v5_hg19/${Population}/1kg.phase3.v5.shapeit2.${pop}.hg19.all.SNPs.uniq_posID  --threads 20 --clump-r2 0.2 --clump-kb 250 --clump $fo1 --out ../output/tissue_pop_clump_0.5/$used_to_clump";
                # print "$command\n";    
                system $command;
                print "$fo1\n";                
            }
            else{
                my $command= "plink --bfile /share/data0/1kg_phase3_v5_hg19/${Population}/1kg.phase3.v5.shapeit2.${pop}.hg19.all.SNPs.uniq.posID  --threads 20 --clump-r2 0.2 --clump-kb 250 --clump $fo1 --out ../output/tissue_pop_clump_0.5/$used_to_clump";
                # print "$command\n";
                system $command;
                print "$fo1\n";
            }           
