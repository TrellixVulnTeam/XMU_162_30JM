source("/home/huanhuan/project/RNA/eQTL_associated_interaction/QTLbase/script/OmicCircos_circos_refine_revise_color.R") # load the fixed function
...
for (i in c(1:22)){
  ...
  data_chr_1<- data_chr[,c("region",'ALL QTL','eQTL','mQTL','sQTL','caQTL','hQTL','pQTL','edQTL')]
  ...
  data_chr_2<-data_chr_qqq[,-c(1,10,11)]
  ...
  figure_name<-paste("p_ed_Chr",i,"orginal.pdf",sep ="_")
  pdf(figure_name)
...
data1<- data[,c("chr","region",'ALL QTL','eQTL','mQTL','sQTL','caQTL','hQTL','pQTL','edQTL')]
...
colnames(scale_data1)[9] <- 'pQTL'
colnames(scale_data1)[10] <- 'edQTL'
#---------------------------------
...
colnames(normalize_data)[9] <- 'pQTL'
colnames(normalize_data)[10] <- 'edQTL'
...
for (i in c(1:22)){
  ...
  color2 = colorRampPalette(c('#c6dbef','#6baed6','#2171b5'))(50)
  title= paste("Chr",i,sep =" ")
  figure_name<-paste("p_ed_Chr",i,"times_scale_original.pdf",sep ="_")
  pdf(figure_name)
  pheatmap(data_used_heatmap,cluster_rows = FALSE, cluster_cols = FALSE,color = color2,fontsize_row=1.5,fontsize_col=4,cellwidth=7,fontsize=5, main =title)
  dev.off()}
...
for (i in c(1:22)){
  ...
  color2 = colorRampPalette(c('#c6dbef','#6baed6','#2171b5'))(50)
  title= paste("Chr",i,sep =" ")
  figure_name<-paste("p_ed_Chr",i,"max_min_scale_original.pdf",sep ="_")
  pdf(figure_name)
  pheatmap(data_used_heatmap,cluster_rows = FALSE, cluster_cols = FALSE,color = color2,fontsize_row=1.5,fontsize_col=4,cellwidth=7,fontsize=5, main =title)
  dev.off()}
...
for (i in c(1:22)){
  ...
  color2 = colorRampPalette(c('#c6dbef','#6baed6','#2171b5'))(50)
  title= paste("Chr",i,sep =" ")
  figure_name<-paste("p_ed_Chr",i,"times_scale_less.pdf",sep ="_")
  pdf(figure_name)
  pheatmap(data_used_heatmap,cluster_rows = FALSE, cluster_cols = FALSE,color = color2,fontsize_row=1.5,fontsize_col=4,cellwidth=7,fontsize=5, main =title)
  dev.off()}
#----------------------------------------------------max min scale less
...
for (i in c(1:22)){
  ...
  color2 = colorRampPalette(c('#c6dbef','#6baed6','#2171b5'))(50)
  title= paste("Chr",i,sep =" ")
  figure_name<-paste("p_ed_Chr",i,"max_min_scale_less.pdf",sep ="_")
  pdf(figure_name)
  pheatmap(data_used_heatmap,cluster_rows = FALSE, cluster_cols = FALSE,color = color2,fontsize_row=1.5,fontsize_col=4,cellwidth=7,fontsize=5, main =title)
  dev.off()}
