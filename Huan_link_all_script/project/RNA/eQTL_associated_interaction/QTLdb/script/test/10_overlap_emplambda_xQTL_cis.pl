    ...
    foreach my $QTL2(@QTL2s){
        unless ($QTL1 =~/$QTL2/){#防止自己与自己取交集
            ...
            unless(exists $hash1{$cp_format1} || exists $hash1{$cp_format2} ){
                ...
                my $fo1 = "../output/xQTL_merge/Cis_1MB/NHPoisson_emplambda_interval_1000_cutoff_7.3_all_merge_cis_1MB_${QTL1}_${QTL2}.txt.gz";
                open my $O1, "| gzip >$fo1" or die $!;
                print $O1 "${QTL1}_emplambda\t${QTL2}_emplambda\tpos\tchr\n";
                my $fo2 = "../output/xQTL_merge/Cis_10MB/NHPoisson_emplambda_interval_1000_cutoff_7.3_all_merge_cis_10MB_${QTL1}_${QTL2}.txt.gz";
