#将../output/01_unique_variant-id_position_hg19.vcf和../output/01_normal_format.txt.gz merge 在一起，得../normalized/02_normalized.txt.gz
#!/usr/bin/perl
use warnings;
use strict; 
use utf8;
...
while(<$I2>){
    ...
    unless(/^Variant_id/){
        ...
        if (exists $hash1{$Variant_id}){ #hg38转hg19成功的id
            ...
            my $Variant_id_hg19 = $Variant_id;
            my $output = "$Variant_id_hg19\t$Rs_id\t$Chr\t$pos_hg19\t$ref\t$alt\t$Effect_allele\t$Other_allele\t$Maf\t$Gene_Version\t$gene_info\t$other_info\tBlood_eQTL_browser";
            ...
            my $output = "$varint_info\t$gene_info\t$other_info\tBlood_eQTL_browser";
            unless(exists $hash2{$output}){
                $hash2{$output} =1;
                print $O1 "$output\n";
            }
