...
while(<$I2>){
    ...
    unless(/^Variant_id/){
        ...
        if (exists $hash1{$Variant_id}){ #hg38转hg19成功的id
            ...
            my $output = "$Variant_id_hg19\t$Rs_id\t$Chr\t$pos_hg19\t$ref\t$alt\t$Effect_allele\t$Other_allele\t$Maf\t$Gene_Version\t$gene_info\t$other_info\teQTL_databases";
            ...
            my $output = "$varint_info\t$gene_info\t$other_info\teQTL_databases";
            unless(exists $hash2{$output}){
                $hash2{$output} =1;
                print $O1 "$output\n";
            }
