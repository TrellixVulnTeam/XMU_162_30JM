...
ProcessBedGz <- function(tissue = NULL){
    ...
    pwd_dir <-paste0("/home/huanhuan/project/RNA/eQTL_associated_interaction/PancanQTL/output/",tissue,"/Trans_eQTL/NHP/")
    dir.create(pwd_dir,recursive = T)
    setwd(pwd_dir)
    org <- read.table(paste0("../../../",tissue,"/",tissue,"_trans_eQTL_1kg_Completion.txt.gz"),header = T,sep = "\t") %>% as.data.frame()
    cutoff =7.3
    num = 18
