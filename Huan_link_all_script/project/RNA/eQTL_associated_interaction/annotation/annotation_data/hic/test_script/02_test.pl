#将hg19/下面的.loops文件合在一起，由这些文件判断，该文件是1-based，改为0-based,判断cis-trans_interaction 并去重得02_all_unique_hic_loops_tran_cis_1.bed.gz(interaction1_interaction2),
...
foreach my $file(@files){
    ...
    while(<$I1>)
    {
        ...
        if($chr1 eq $chr2 && $different <=1000000){ #1MB_cis
            my $final_output2 ="$output2\tcis_1MB";
            my $final_output3 ="$output3\tcis_1MB";
            unless(exists $hash2{$final_output2}){
                $hash2{$final_output2} =1;
                print $O2 "$final_output2\n";
            }
            unless(exists $hash3{$final_output3}){
                $hash3{$final_output3} =1;
                print $O3 "$final_output3\n";
            }
        }
        else{ ##1MB_trans
            my $final_output2 ="$output2\ttrans_1MB";
            my $final_output3 ="$output3\ttrans_1MB";
            unless(exists $hash2{$final_output2}){
                $hash2{$final_output2} =1;
                print $O2 "$final_output2\n";
            }
        ...
        if ($chr1 eq $chr2 && $different <=10000000){ #10MB_cis
            my $final_output2 ="$output2\tcis_10MB";
            my $final_output3 ="$output3\tcis_10MB";
            unless(exists $hash2{$final_output2}){
                $hash2{$final_output2} =1;
                print $O2 "$final_output2\n";
            }
            ...
            my $final_output2 ="$output2\ttrans_10MB";
            my $final_output3 ="$output3\ttrans_10MB";
            unless(exists $hash2{$final_output2}){
                $hash2{$final_output2} =1;
                print $O2 "$final_output2\n";
            }
