...
foreach my $dir(@dir1s){
    ...
    foreach my $TCGA(sort keys %hash2){
        print "$TCGA\n";
        my @vs= @{$hash2{$TCGA}};
        my $final_out_dir= "./cancer_cell/${out_dir}/${TCGA}";
        unless(-e $final_out_dir){
            system "mkdir -p $final_out_dir";
        }
        my $fo1 = "${final_out_dir}/merge_pos_info_sample_narrow_peak.bed.gz";
        # open my $O1, '>', $fo1 or die "$0 : failed to open output file '$fo1' : $!\n";
        open my $O1, "| gzip >$fo1" or die $!;
        my $fo2 = "${final_out_dir}/merge_pos_info_narrow_peak.bed.gz";
        ...
        if($factor_line_count >0){
            ...
            my $command1 = "zless $fo2 |sort -k1,1 -k2,2n |gzip >$out1 ";
