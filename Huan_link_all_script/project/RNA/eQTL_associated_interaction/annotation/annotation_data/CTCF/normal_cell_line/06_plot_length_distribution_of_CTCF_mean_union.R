...
org<-read.table("05_normal_cell_line_ctcf_sort_union.bed.gz",header = F,sep = "\t") %>% as.data.frame()
...
colnames(org)[3] <-"end"
write.table(org,"06_CTCF_length_union.txt",row.names = F, col.names = T,quote =F,sep="\t")
...
write.table(rs,"06_CTCF_length_statistics_union.txt",row.names = F, col.names = T,quote =F,sep="\t")
