...
setwd("/home/huanhuan/project/RNA/eQTL_associated_interaction/annotation/annotation_data/ROADMAP/Lung/enhancer/")
    # for (type in types){
        # output_file2<-paste("NHPoisson_emplambda_interval_",j,"_cutoff_",cutoff,"_",tissue,".txt", sep = "")
org<-read.table("01_normal_E096-H3K4me1_H3K27ac.narrowPeaksorted_overlap.gz",header = F,sep = "\t") %>% as.data.frame()
