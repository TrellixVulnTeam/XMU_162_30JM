...
rs1 <- filter(rs,Random_number <101)
#-------------two side
all_fisher <-data.frame()
for(state in unique(hotspot$Marker)){
    aa <-filter(rs1,Marker==state)%>%filter(jaacard_index >0)%>%group_by(Class)%>%summarise(table(Class)%>%as.data.frame())%>%as.data.frame()
    a <-filter(aa,Class=="hotspot")$Freq
    r_c<-filter(aa,Class=="random")$Freq
    pre_data <-matrix(c(a,r_c, nrow(hotspot)/7-a, nrow(hotspot)/7*100-r_c), nrow = 2)
    ...
    tmp <-data.frame(marker =state,  p_value =p_value,conf_int_bottom=conf_int1,conf_int_up=conf_int2,OR = od)
    all_fisher <- bind_rows(all_fisher,tmp)
    print(state)
}

write.table(all_fisher,"./figure/0_0.176/compare_0_0.176_jaacard_index_fisher_test_two_side_100.txt",row.names = F, col.names = T,quote =F,sep="\t")
...
write.table(all_fisher,"./figure/0_0.176/compare_0_0.176_jaacard_index_fisher_test_greater_100.txt",row.names = F, col.names = T,quote =F,sep="\t")
