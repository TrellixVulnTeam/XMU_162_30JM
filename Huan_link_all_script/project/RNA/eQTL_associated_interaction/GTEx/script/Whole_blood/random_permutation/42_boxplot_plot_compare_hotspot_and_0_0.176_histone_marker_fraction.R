...
random<-read.table("background_original_random_0_0.176_histone_marker.txt.gz",header = T,sep = "\t") %>% as.data.frame()

hotspot$random_number <- 1
hotspot$Class <- "Hotspot"
random$Class <- "Random expectaion"
hotspot$Fraction <- hotspot$count/46369
random$Fraction <-random$count/46369
...
pdf("./figure/percentage/compare_emplambda0_0.176_all_histone_marker_fraction.pdf",width=8.3, height=4)
CombinePlots(plist,ncol=4,nrow=2)
dev.off()
...
pdf("./figure/percentage/compare_emplambda0_0.176_all_histone_marker_tfbs_fraction.pdf",width=8.6, height=6)
CombinePlots(plist,ncol=4,nrow=3)
dev.off()

plist = lapply(other_marker,large)
pdf("./figure/percentage/compare_emplambda0_0.176_other_tfbs_fraction.pdf",width=6, height=2)
...
write.table(final_result,"./figure/percentage/compare_emplambda0_0.176_all_histone_marker_tfbs_fraction_fold_change.txt",row.names = F, col.names = T,quote =F,sep="\t")
