...
pdf("./figure/count/15_state_active_chromatin.pdf",width=8.3, height=4)
CombinePlots(plist,ncol=4,nrow=2)
dev.off()

plist = lapply(bivalent_chromatin,large)

pdf("./figure/count/15_state_bivalent_chromatin.pdf",width=6, height=2)
CombinePlots(plist,ncol=3)
dev.off()

plist = lapply(silent_chromatin1,large)

pdf("./figure/count/15_state_silent_chromatin1.pdf",width=6, height=2)
CombinePlots(plist,ncol=3)
dev.off()
...
pdf("./figure/count/15_state_Quies.pdf",width=2.2, height=2)
# CombinePlots(plist,ncol=3)
plist
dev.off()
