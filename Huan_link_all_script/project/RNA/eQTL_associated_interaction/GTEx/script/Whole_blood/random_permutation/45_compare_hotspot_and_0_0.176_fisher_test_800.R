...
rs1 <- filter(rs,Random_number <801)
...
for(state in marks){
    ...
    pre_data <-matrix(c(a,r_c, nrow(hotspot)/7-a, nrow(hotspot)/7*800-r_c), nrow = 2)
...
write.table(all_fisher,"./figure/0_0.176/compare_0_0.176_jaacard_index_fisher_test_two_side_800.txt",row.names = F, col.names = T,quote =F,sep="\t")
...
write.table(all_fisher,"./figure/0_0.176/compare_0_0.176_jaacard_index_fisher_test_greater_800.txt",row.names = F, col.names = T,quote =F,sep="\t")
