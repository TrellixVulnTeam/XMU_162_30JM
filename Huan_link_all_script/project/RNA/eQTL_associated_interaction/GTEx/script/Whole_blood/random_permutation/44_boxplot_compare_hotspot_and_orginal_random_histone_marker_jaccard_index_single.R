...
large <-function(state){
    ...
    title_name<-state
    figure_name <- paste0("./figure/original_random/",state,"_jaccard_index.pdf")
    pdf(figure_name,width=3.5, height=3.5)
...
large <-function(state){
    aa <-filter(rs,Marker==state)
    title_name<-state
    figure_name <- paste0("./figure/original_random/",state,"_log2_1_jaccard_index.pdf")
    pdf(figure_name,width=2.1, height=2.25)
    p <-ggplot(aa,aes(x=Class,y=log2_1_jaccard_index,fill = Class))+ geom_violin(outlier.colour = NA)+ theme(legend.position ="none") +ggtitle(title_name) +theme(plot.title = element_text(hjust = 0.5))+ylab("Log2(1+ Jaccard index)")+p_theme+stat_compare_means(comparisons = my_comparisons, method = "wilcox.test", method.args = list(alternative = "greater"))
    print(p)
    dev.off()
}
...
plist = lapply(repressed_mark,large)
    ...
    aa <-filter(rs,Marker==state)
    title_name<-state
    figure_name <- paste0("./figure/original_random/",state,"_log2_1_jaccard_index2.pdf")
    pdf(figure_name,width=2.1, height=2.25)
...
# pdf("./figure/original_random/compare_original_active_mark_jaccard_index.pdf",width=6.3, height=4.5)
# CombinePlots(plist,ncol=3,nrow=2)
# dev.off()

# plist = lapply(repressed_mark,large)

# pdf("./figure/original_random/compare_original_repressed_mark_jaccard_index.pdf",width=4, height=2.25)
