...
random<-read.table("background_original_random_0_histone_marker.txt.gz",header = T,sep = "\t") %>% as.data.frame()
...
pdf("./figure/percentage/compare_emplambda0_all_histone_marker_percentage.pdf",width=8.3, height=4)
CombinePlots(plist,ncol=4,nrow=2)
dev.off()
...
pdf("./figure/percentage/compare_emplambda0_all_histone_marker_tfbs_percentage.pdf",width=8.3, height=6)
CombinePlots(plist,ncol=4,nrow=3)
dev.off()

plist = lapply(other_marker,large)
pdf("./figure/percentage/compare_emplambda0_other_tfbs_percentage.pdf",width=6, height=2)
