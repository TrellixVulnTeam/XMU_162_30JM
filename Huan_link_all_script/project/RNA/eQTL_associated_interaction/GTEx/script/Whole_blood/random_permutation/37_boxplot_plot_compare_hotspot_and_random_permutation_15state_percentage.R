...
random<-read.table("random_permutation_15_state_count_Whole_Blood_cutoff_0.176.txt",header = T,sep = "\t") %>% as.data.frame()
...
pdf("./figure/random_permutation/15_state_active_chromatin.pdf",width=8, height=4)
CombinePlots(plist,ncol=4,nrow=2)
dev.off()

plist = lapply(bivalent_chromatin,large)

pdf("./figure/random_permutation/15_state_bivalent_chromatin.pdf",width=6, height=2)
CombinePlots(plist,ncol=3)
dev.off()

plist = lapply(silent_chromatin1,large)

pdf("./figure/random_permutation/15_state_silent_chromatin1.pdf",width=6, height=2)
CombinePlots(plist,ncol=3)
dev.off()
...
pdf("./figure/random_permutation/15_state_Quies.pdf",width=2, height=2)
# CombinePlots(plist,ncol=3)
plist
dev.off()
