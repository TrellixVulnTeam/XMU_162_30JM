...
pdf("fisher_test_0_0.176_jaacard_index_forest_plot.pdf",height = 3.5,width = 4) #
...
p1<-ggplot(data=org, aes(x=marker,y=OR, ymin=conf_int_bottom , ymax=conf_int_up))
  ...
  p1<-p1+ylab("Odds ratio")+xlab("Markers") 
  p1 <-p1+p_theme+theme(axis.text.x = element_text(color = "black"),axis.text.y = element_text(color = "black"))
  p1 <-p1+geom_text(aes(x=marker,y=2.85,label = significant)) 
}

# pdf("fisher_test_0_0.176_jaacard_index_forest_plot.pdf",height = 3.2,width = 4)
pdf("fisher_test_0_0.176_jaacard_index_forest_plot_ORG.pdf",height = 3.5,width = 4) #
org$marker <- factor(org$marker, levels=org$marker) #marker ranking
p<-credplot.gg_main(org)
p
dev.off()