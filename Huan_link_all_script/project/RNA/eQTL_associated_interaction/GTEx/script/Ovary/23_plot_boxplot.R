...
#-------------------------------------------------------
tissues=c("Liver","Lung","Pancreas","Whole_Blood","Ovary")
# tissue = "Liver"
for(tissue in tissues){
    org<-read.table(paste0("/home/huanhuan/project/RNA/eQTL_associated_interaction/GTEx/output/",tissue,"/Cis_eQTL/22_",tissue,"_cis_eQTL_in_hotspot_ratio.gz"),header = T,sep = "\t") %>% as.data.frame()
    ...
    p1 <-p1 + ggtitle(tissue)+theme(plot.title = element_text(hjust = 0.5))
    print(p1)
    dev.off()
    print(tissue)
}
