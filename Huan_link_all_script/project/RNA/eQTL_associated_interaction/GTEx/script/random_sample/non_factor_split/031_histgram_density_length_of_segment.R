...
ProcessBedGz <- function(j = NULL){
    for(group in groups){
        ...
        for (type in types){ 
            ...
            max_limit <- max(org$count)
            min_limit <-min(org$count)
        ...
        pdf(paste0(group,"/Density_plot_length_of_segment_",group,"_cutoff_7.3_interval_",j,".pdf")) 
        ...
        pdf(paste0(group,"/Histogram_plot_length_of_segment_",group,"_cutoff_7.3_interval_",j,".pdf")) 
