...
for(tissue in tissues){
    setwd(paste0("/share/data0/QTLbase/huan/GTEx/random_select/",tissue,"/figure/"))
    for(number in nums){
        print(number)
        figure_list <-list ()
        i=1
        for(cutoff in cutoffs){
            file_name<-paste0("/share/data0/QTLbase/huan/GTEx/random_select/",tissue,"/hotspot/",number,"/interval_18/",tissue,"_segment_hotspot_cutoff_",cutoff,".bed.gz")
