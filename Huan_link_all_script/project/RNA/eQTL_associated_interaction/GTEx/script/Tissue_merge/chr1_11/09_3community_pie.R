...
org<-read.table("communities_3.bed.gz",header =F,sep="\t")%>%data.frame()
colnames(org)[4] <-"Group"
mycolor<-c( "#70A1D7","#C86B85","#FFD2A5", "#C79ECF")
a<-table(org$Group)%>%as.data.frame()


setwd("/home/huanhuan/project/RNA/eQTL_associated_interaction/GTEx/script/Tissue_merge/chr1_11/6kmer/3_community")
