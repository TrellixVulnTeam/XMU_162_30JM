            ...
            unless(/^Cutoff/){
                ...
                if(abs($cutoff -$Cutoff)<0.0000001){
                    ...
                    if (exists $hash1{$Chr}){
                        ...
                        # my $fo1 = "./tmp/chr_size_hh.txt";
                        # # open my $O1, "| gzip >$fo1" or die $!;
                        # open my $O1, '>', $fo1 or die "$0 : failed to open output file '$fo1' : $!\n";
                        # print $O1 "$Chr\t$chr_size\n";
                        # my $command2 = "bedtools makewindows -g  ./tmp/chr_size_hh.txt -n $hotspot_Number | gzip > /share/data0/QTLbase/huan/GTEx/random_segment/mean_hotspot/split_chr_size/split_${Cutoff}_${Chr}_${tissue}.bed.gz";
                        # system $command2;
                        my $k3 = "$Cutoff\t$Chr\t$tissue"; #------不同chr,有些cutoff是不存在的，为后面判断做铺垫
                        $hash3{$k3}=1;
                        # print "$Cutoff\t$Chr\t$tissue\n";
                    }
                }
            }
        }
    #----------------------
        ...
        for(my $i=1; $i<11;$i++){
            ...
            foreach my $k(keys %hash2){
                ...
                if (exists $hash3{$kk3}){  #不同chr,有些cutoff是不存在的
                    ...
                    while(<$I2>){
                        ...
                        my $start = $f[1];
                        my $end = $f[2];
                        my $allow_end_to_random = $end - $Average_Length;
