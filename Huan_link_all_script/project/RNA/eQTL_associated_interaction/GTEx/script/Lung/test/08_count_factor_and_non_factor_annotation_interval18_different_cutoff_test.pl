...
print $O1 "cutoff\tFactor\tNumber_of_factor_in_hotspot\tNumber_of_factor_in_non_hotspot\tNumber_of_non_factor_in_hotspot\tNumber_of_non_factor_in_non_hotspot\tTPR\tFPR\n";
foreach my $factor(@factors){
    ...
    foreach my $cutoff(@cutoffs){
        ...
        my $command_factor_hotspot = "zless ../output/Whole_Blood/Cis_eQTL/annotation/interval_18/ALL/factor/hotspot/${cutoff}/${factor}_whole_blood_segment_hotspot_cutoff_${cutoff}.bed.gz|cut -f4,5,6| sort -u | wc -l" ;
        my $factor_hotspot_line_count = wc($command_factor_hotspot);
        my $command_non_factor_hotspot = "zless ../output/Whole_Blood/Cis_eQTL/annotation/interval_18/ALL/non_factor/hotspot/${cutoff}/non_${factor}_whole_blood_segment_hotspot_cutoff_${cutoff}.bed.gz|cut -f4,5,6| sort -u | wc -l" ;
        my $non_factor_hotspot_line_count = wc($command_non_factor_hotspot);
        my $TP = $factor_hotspot_line_count;
        my $FP = $non_factor_hotspot_line_count;
        my $FN = $factor_line_count - $TP;
        my $TN =  $non_factor_line_count - $FN;
        my $tpr = $TP/($TP+$FN);
        my $fpr= $FP/($FP+$TN);
        print $O1 "$factor\t$cutoff\tt$TP\t$FN\t$FP\t$TN\t$tpr\t$fpr\n";
