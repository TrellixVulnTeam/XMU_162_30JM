###../../output/Whole_Blood/Cis_eQTL/NHP/NHPoisson_emplambda_interval_${j}_cutoff_7.3_Whole_Blood.txt.gz 时得不同cutoff下的non hotspot(segment),../../output/${tissue}/Cis_eQTL/non_hotspot_cis_eQTL/interval_${j}/${tissue}_segment_non_hotspot_cutoff_${cutoff}.bed.gz
#!/usr/bin/perl
use warnings;
use strict; 
use utf8;
...
foreach my $j(@interval){
    ...
    foreach my $cutoff(@cutoffs){
        ...
        my $fo3 = ".././output/${tissue}/Cis_eQTL/non_hotspot_cis_eQTL/interval_${j}/${tissue}_segment_non_hotspot_cutoff_${cutoff}.bed.gz";
