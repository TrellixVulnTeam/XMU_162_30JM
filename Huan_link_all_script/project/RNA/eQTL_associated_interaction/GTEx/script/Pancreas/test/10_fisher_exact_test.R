...
for(type in types){
  ...
  org1<-read.table("09_prepare_number_ROC.txt",header =  T,sep = "\t") %>% as.data.frame()
  ...
  write.table(rs,"10_fisher_exact_test_result.txt",row.names = F, col.names = T,quote =F,sep="\t")
}
