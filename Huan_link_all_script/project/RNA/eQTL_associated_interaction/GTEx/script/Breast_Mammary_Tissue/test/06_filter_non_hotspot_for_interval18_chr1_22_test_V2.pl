###../output/Whole_Blood/Cis_eQTL/NHP/NHPoisson_emplambda_interval_${j}_cutoff_7.3_Whole_Blood.txt.gz 时得不同cutoff下的hotspot(segment),"../output/Whole_Blood/Cis_eQTL/hotspot_cis_eQTL/interval_${j}/whole_blood_segment_hotspot_cutoff_${cutoff}_chr1_22.bed.gz"
#!/usr/bin/perl
use warnings;
use strict; 
use utf8;
...
my @cutoffs;
# for (my $i=0.55;$i<0.95;$i=$i+0.05){ #对文件进行处理，把所有未定义的空格等都替换成NONE
#     push @cutoffs,$i;
#     # print "$i\n";
# }
# push @cutoffs,0.01;
...
foreach my $j(@interval){
    ...
    while(<$I1>)
    {
        ...
        unless(/emplambda/){
            ...
            my $v= "$pos\t$emplambda";
            # print "$chr\n";
            if ($chr=~/\b1\b/ || $chr=~/\b22\b/){
            # if ($chr eq 1 || $chr eq 22){
                # print "$chr\n";
                unless($emplambda =~/NA/){
                    # if ($chr == 1){
                    push @{$hash1{$chr}},$v;
    ...
    foreach my $cutoff(@cutoffs){
        ...
        # my $fo3 = "../output/Whole_Blood/Cis_eQTL/hotspot_cis_eQTL/interval_${j}/whole_blood_segment_hotspot_cutoff_${cutoff}_chr1_22.bed.gz";
        my $fo3 = "../output/Whole_Blood/Cis_eQTL/non_hotspot_cis_eQTL/interval_${j}/whole_blood_segment_non_hotspot_cutoff_${cutoff}_chr1_22.bed.gz";
                ...
                if ($emplambda < $cutoff){
                    ...
                    if ($result_count <2){ #第一个大于cutoff 的值
                        ...
                        if ($end_diff <2 ){ #--------------和前面end行数相差1,和前面的区域相连
                            ...
                            # print "$count\t2222\n";
                            my $length = @vs;
                            if($length == $count){ #最后一个non_hotspot的最后一行
                                my $bed_ends = $ends[0]+1;
                                print $O3 "chr${chr}\t$starts[0]\t$bed_ends\n";
                                print "ALL Non_hotspot END\n";
                            }
                        }
