    ...
    foreach my $type(@types){
        foreach my $group(@groups){
            foreach my $fraction(@fractions){
                ...
                my $output_dir = "/state/partition1/huan/QTLdb/output/ALL_eQTL/cis_trans/fisher_exact_test_extend_100bp/annotation/interval_${interval}/${group}/${type}/${fraction}"; #compute-0-1
                # mkdir $PMID;
                # #------------
                if(-e $output_dir){
                ...
                my $command = "bash annotation_bedtools_intersect_v2.sh";
                # print "$command\n";
                system $command;
