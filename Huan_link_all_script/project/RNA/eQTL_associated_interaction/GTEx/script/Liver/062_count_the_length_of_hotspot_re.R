...
for(cutoff in cutoffs){
    ...
    if (a > 20){ #the null compressed files is 20
        ...
        org_filter <-filter(org,hotspot_length >6)
        ...
        # figure_list[[i]] <-ggplot(bbb, aes(x =Var1,y=log10(Freq))) +geom_histogram(stat="identity")+xlab("Length of segment") + ylab("Log(Count)")+p_theme+ggtitle(title_name)+
        # figure_list[[i]] <-ggplot(org, aes(x =hotspot_length)) +geom_bar(position="identity")+scale_y_log10()+xlab("Length of segment") + ylab("Log(Count)")+p_theme+ggtitle(title_name)+
