...
univ_models <- lapply( univ_formulas, function(x){coxph(x, data = prog_not_fill)})
...
result1$p.value <-as.numeric(as.character(result1$p.value))
sig_result1 <-filter(result1,p.value <0.05)
write.table(file="01_not_fill_univariate_cox_result.txt",result1 ,quote=F,sep="\t",row.names=FALSE)
...
res.cox <- coxph(Surv(pod.total) ~ gender + ECOG + stage +Bsymptom + Lymph.node+ spleen +BM +transform+ PRIME.PI +end.evaluation +R.course+ progress + HGB.g.l.+ MONO + SPD, data =  prog_not_fill)
...
write.table(file="01_not_fill_multivariate_cox_result.txt",multi_res,quote=F,sep="\t")

write.table(file="01_not_fill_multivariate_cox_result_sig.txt",multi_res_sig,quote=F,sep="\t")
